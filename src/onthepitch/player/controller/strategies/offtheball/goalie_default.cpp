			...
			if (controller->GetFadingTeamPossessionAmount() < 1.0f) 
			{
				...
				{ 
					...
					if (oppHelper) 
					{
						...
						if (mateHelper) 
							mateHelperToThresholdDistance = (helperShootingPoint - (mateHelper->GetPosition() + mateHelper->GetMovement() * 0.24f)).GetLength();
						...
						if (mateHelperToThresholdDistance > oppHelperToThresholdDistance) 
							secondaryDistanceDiff = NormalizedClamp(mateHelperToThresholdDistance - oppHelperToThresholdDistance, 0.0f, 2.0f);
...
*/
		...
		if (fabs(intersectPoint.coords[1]) > 3.7 * panic) 
			intersect = false; else intersect = true;
